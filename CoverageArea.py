...
def mark_points(sensor, r, points, sensors_coverage):
    left, right, down, up = max(sensor.x - r, 0), min(sensor.x + r + 1, 1080), max(sensor.y - r, 0), min(sensor.y + r + 1, 720)
    sensor_coverage_count = 0
    r2 = r ** 2
    
    for i in range(left, right):
        for j in range(down, up):
            if not points[i, j] and (sensor.x - i) ** 2 + (sensor.y - j) ** 2 <= r2:
                points[i, j] = True
...
def convert_to_SENSOR(locations, n_bits):
    ...
    for location in locations:
        x_axis = convert_from_binlist_to_int(location[:bits_no_for_axis])
        y_axis = convert_from_binlist_to_int(location[bits_no_for_axis:])
...
def calculate(sensors_locations, n_bits, CRPS):
    ...
    # Using numpy array for efficient point marking
    points = np.zeros((1080, 720), dtype=bool)
    sensors_coverage = []
